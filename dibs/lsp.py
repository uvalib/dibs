...
class VirgoAPIInterface(LSPInterface):
    ...
    def getGuestAuthKey(self):
            ...
            if (self.secret != None):
                ...
                values =  {'userId': 'anonymous', 'isUva': False, 'homeLibrary': '', 'profile': '', 'canPurchase': False, 'canLEO': False, 'canLEOPlus': False, 'canPlaceReserve': False, 'useSIS': False, 'role': 'guest', 'authMethod': 'none', 'version': '1.2.3', 'exp': expirationTime, 'iss': 'v4'}
    ...
    def getAuthKeyForPerson(self, username = None):
            ...
            if (username != None and self.secret != None):
                expirationTime = int((datetime.now() + timedelta(minutes=20)).timestamp())
                values = {"iss" : "v4", "userId" : f'{username}', "version": "1.2.3", "exp" : expirationTime}
